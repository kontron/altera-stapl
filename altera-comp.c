...
static uint32_t altera_bits_req(uint32_t n)
{
	uint32_t result = SHORT_BITS;
...
static uint32_t altera_read_packed(uint8_t *buffer, uint32_t bits, uint32_t *bits_avail,
							uint32_t *in_index)
{
	uint32_t result = 0;
	uint32_t shift = 0;
...
{
	uint32_t i, j, data_length = 0L;
	uint32_t offset, length;
	uint32_t match_data_length = MATCH_DATA_LENGTH;
	uint32_t bits_avail = CHAR_BITS;
	uint32_t in_index = 0L;
	...
	while (i < data_length) {
		...
		if (altera_read_packed(in, 1, &bits_avail,
			...
			for (j = 0; j < DATA_BLOB_LENGTH; ++j) {
				if (i < data_length) {
					out[i] = (uint8_t)altera_read_packed(in,
							CHAR_BITS,
							&bits_avail,
							&in_index);
					i++;
			...
			offset = altera_read_packed(in, altera_bits_req((int16_t)
					(i > match_data_length ?
						match_data_length : i)),
					&bits_avail,
					&in_index);
